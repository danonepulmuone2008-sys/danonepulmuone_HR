...
export default function AttendancePage() {
  ...
  const fetchMonthData = useCallback(async (uid: string) => {
    ...
    const reqList: RequestItem[] = [
      ...(vacations ?? []).map(v => ({ id: v.id, type: "vacation" as const, label: v.type, date: v.start_date, status: statusKo(v.status) })),
      ...(trips ?? []).map(t => ({ id: t.id, type: "business_trip" as const, label: t.destination, date: t.start_date, status: statusKo(t.status) })),
  ...
  const handleFlexSubmit = () => {
    ...
    setMyFlexSchedules(prev => [...prev.filter(s => s.date !== dateStr), { date: dateStr, startTime: flexInput.startTime, endTime: flexInput.endTime }]);
    setSelectedDay(null);
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 overflow-visible">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <label className="flex items-center gap-1.5 cursor-pointer select-none">
                <input type="checkbox" checked={showTeam} onChange={e => setShowTeam(e.target.checked)} className="w-3.5 h-3.5 accent-blue-600" />
                <span className="text-xs text-gray-500">팀 일정</span>
              </label>
            </div>
          </div>
          <div className="grid grid-cols-7 text-center mb-1">
            {CALENDAR_DAYS.map(d => <span key={d} className="text-xs text-gray-400 font-medium py-1">{d}</span>)}
          </div>
                ...
                {week.map((day, di) => {
                  ...
                  return (
                    <div key={di} className="relative flex flex-col items-center py-0.5 group" onClick={() => { if (day) { setSelectedDay(day); setModalMode("detail"); } }}>
                      <div className={`text-sm rounded-full w-7 h-7 flex items-center justify-center ${day === todayDate ? "bg-blue-600 text-white font-bold" : day ? "text-gray-700 hover:bg-gray-100 cursor-pointer" : ""}`}>
                        {day ?? ""}
                      </div>
                      ...
                      <div className="flex gap-0.5 mt-0.5 h-2 items-center justify-center">
                        {events.slice(0, 3).map((ev, ei) => (
                          <span key={ei} className={`w-1.5 h-1.5 rounded-full ${ev.type === "vacation" ? "bg-green-500" : "bg-blue-500"}`} />
                        ))}
                      </div>
                      {hasTooltip && (
                        <div className={`absolute bottom-full ${tooltipAlign} mb-2 z-50 hidden group-hover:block bg-gray-900 text-white rounded-xl shadow-xl w-48 p-3 pointer-events-none`}>
          ...
          <div className="flex flex-wrap gap-3 mt-3 pt-3 border-t border-gray-100">
            ...
            {hasFlexDisplay && <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-purple-400 inline-block" /><span className="text-xs text-gray-400">유연근무</span></div>}
          </div>
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40" onClick={closeModal}>
            <div className="bg-white rounded-t-2xl w-full max-w-[390px] pb-10" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-gray-100">
                <div className="flex items-center gap-2">
                  {modalMode === "flex-add" && <button onClick={() => { setModalMode("detail"); setFlexInput({ startTime: "", endTime: "" }); }} className="text-gray-400 hover:text-gray-600 mr-1">←</button>}
                  <div>
                    <h3 className="text-base font-bold text-gray-900">{modalMode === "detail" ? `${currentMonth + 1}월 ${selectedDay}일` : "유연근무 등록"}</h3>
                    <p className="text-xs text-gray-400 mt-0.5">{modalMode === "detail" ? "해당 날의 전체 일정" : "나의 근무 시간을 입력하세요"}</p>
                  </div>
                </div>
              ...
              {modalMode === "detail" ? (
                <div className="px-5 pt-4">
                    ...
                    <div className="mb-4">
                      ...
                      {dayFlex.map((fe, fi) => (
                        <div key={fi} className="flex items-center justify-between py-2 px-3 bg-purple-50 rounded-xl">
                          <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-purple-500 flex-shrink-0" /><span className="text-sm font-semibold text-gray-800">나</span></div>
                  ...
                  {dayFlex.length === 0 && dayEvents.length === 0 && <p className="text-sm text-gray-400 text-center py-6">등록된 일정이 없습니다</p>}
                  <button onClick={() => { if (myFlexForDay) setFlexInput({ startTime: myFlexForDay.startTime, endTime: myFlexForDay.endTime }); setModalMode("flex-add"); }} className="w-full py-3.5 mt-2 mb-1 bg-purple-600 text-white rounded-2xl text-sm font-semibold active:scale-95 transition-all">
